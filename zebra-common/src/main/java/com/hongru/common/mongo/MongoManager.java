	...
	public static MongoClient getMongos() {
		...
		if (mongo == null) {
			try {
				...
				InputStream in = new BufferedInputStream(MongoManager.class.getResourceAsStream("/info.properties"));
				try {
					pro.load(in);
				} catch (IOException e) {
					e.printStackTrace();
				...
				if (usernames == null || (usernames.length ==1 && (null == usernames[0] || usernames[0].equals("")))) {
					//无验证
